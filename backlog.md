...
- **done (2026-06-24)** — **Full pull + audit trail.** Cracked the eLibrary `GetFileListFromP8` JSON API
  ...
  cards in Discourse.
- **medium** — Extend the flagship summaries to the **full 273** (bodies, not just inventory): the
  remaining ~264 PDFs each need a navigate+click download (the iframe batch path is blocked by Chrome's
  multiple-download protection, and `DownloadP8File` is WAF/auth-hardened), then fitz extraction and a
  structured summary. A few are scanned (e.g., Data Center Coalition) and need OCR. Re-run
  `build-comment-audit.mjs` as summaries land; the categorization auto-aggregates.
