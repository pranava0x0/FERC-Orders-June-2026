...
- **Size a fan-out to the user's *session* budget, and stop the *unstarted* work when told.** A multi-agent run can hit the 5-hour rate limit, which locks the user out for the rest of the window — a real cost. Run large corpus jobs in **small, disk-resumable chunks** (≈3–10, not 40); report token spend per chunk; pause for an explicit go-ahead. (One run launched 40 comments ≈ 1M tokens against a ~9% budget and hit the wall at comment 23.) See [[respect-session-budget]].
- **The expensive moment is the *spawn*, not the run — so don't reflexively kill running agents.** Most of a subagent's cost is its setup (context-load + orchestration, the ~25–40K), paid up front the instant it starts; the generation that follows is comparatively cheap. So `TaskStop` recovers real tokens **only for agents that haven't started yet** (setup unpaid). Killing an agent already mid-generation wastes the setup you *already* paid **and** discards a nearly-finished result — pure loss. When the user flags an over-large fan-out: stop the not-yet-started agents, let the already-running ones finish, and use their output. The fix is upstream: **decide carefully before spawning** (scale agent count to the task's risk; personas are *perspectives*, not a reason for one agent each), because that decision is where the money is spent.
