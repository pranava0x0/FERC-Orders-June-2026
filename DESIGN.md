...
- **The `<details>`-collapse trap (sibling of the `[hidden]` trap in § 11.x).** Native collapse works by a UA rule `details:not([open]) > *:not(summary) { display: none }` — but any **author** `display:` on the body (`.section{display:grid|flex}`) *outranks* the UA sheet, so the closed section keeps rendering full-height (one tab measured *bigger* after "collapsing"). Ship an explicit, specific-enough override beside any accordion: `details.acc:not([open]) > :not(summary) { display: none }`. Verify a collapsed section is actually summary-height (~50px), not its full content.
- **A collapsed-section heading still needs to be a heading.** Folding a `head()`-style `<h2>` section into a `<summary>` and styling the title as a `<span>` silently drops it from the document outline and screen-reader heading navigation. Keep the semantics: put a real `<h2>` inside the summary, or give the styled span `role="heading" aria-level="2"`. Prefer the span+role form when the summary is a flex toggle row — a block `<h2>` as a flex item complicates the summary's baseline sizing.
- **Don't trust a layout measurement taken right after reload or a tab switch.** `getBoundingClientRect()` / computed heights read *mid-reflow* before web fonts and flex/grid settle return phantom values (a 29 px heading measured 800 px; a 54 px collapsed section measured 700 px) — leading you to "fix" a bug that isn't there. Let layout settle first (a real delay between the navigation and the measure, or `await document.fonts.ready` + two `requestAnimationFrame`s), and confirm a surprising number with a second read or a screenshot before acting on it.
...
- **Color a stance grid by *net* sentiment, not the plurality stance.** A stakeholder-by-reform heatmap that colors each cell by its single most-common stance flattens to near-monochrome and hides the friction (the corpus is broadly supportive, so "support" wins almost every cell). Color by a **net score** (e.g. `strong=+2, support=+1, mixed=0, oppose=−1`, averaged) banded into strong-support / support / contested / oppose, so the few genuinely split cells pop. Band thresholds are a judgement call — tune them against the real distribution, not a guess.
- **Don't show a legend swatch for a state with zero instances.** The inverse of "seed one example per enum value": if no cell is *net-oppose*, a "net oppose" key in the legend advertises data that isn't there and reads as if opposition is represented when it isn't. Render legend entries only for states that actually occur in the data (compute the present-set, filter the legend), or state the absence explicitly ("no stakeholder type is net-oppose"). Especially important when the underlying summaries are AI-extracted and known to under-select opposition — an empty "oppose" slot is then ambiguous between "none exists" and "extractor missed it."
